...
*/
    ...
    */
    getRefValue : function (ref) {
        var val;
        for (var i = 0; i < this.rovCells.length; i++) {
            if (this.rovCells[i].viewType == ref.viewType &&
                this.rovCells[i].primaryKeyColumnName == ref.primaryKeyColumnName &&
                this.rovCells[i].primaryKeyColumnValue == ref.primaryKeyColumnValue &&
                this.rovCells[i].selectedColumnName == ref.selectedColumnName) {

                val = this.rovCells[i].val;
                break;
            }
        }
        return (val);
    ...
    *  ======== hasOtherReference ========
    */
    hasOtherReference : function (cellRef) {
        /* if cellRef has a reference other than cellRef.displayColumnName */
        var hasRef = false;
        for (var i = 0; i < this.rovCells.length; i++) {
            if (this.rovCells[i].displayColumnName != cellRef.displayColumnName) {
                var pattern = RegExp(this.rovCells[i].displayColumnName);
                if (cellRef.expression.match(pattern)) {
                    hasRef = true;
                    break;
                }
            }
