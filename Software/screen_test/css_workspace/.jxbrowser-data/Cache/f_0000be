Polymer({ is : "ti-rov-panel",
    ...
    customViewFromGraph : function(trace) {
        ...
        if (foundElem) {
            ...
            var newElem = this.newRovCustomView({tableName: trace.viewName}, trace.origCells);
    ...
    customViewNameChanged : function (prevName, newName) {
        for (var i = 0; i < this.plotViews.length; i++) {
            for (var j = 0; j < this.plotViews[i].traces.length; j++) {
                ...
                if (trace.isCustom && trace.viewName == prevName) {
        ...
        if (error == null) {
            ...
            for (var i = 0; i < keys.length; i++) {
                ...
                for (var j = 0; j < argsKeys.length; j++) {
                    ...
                    for (k = 0; k < viewArg.args.length; k++) {
    ...
    matchPlotViews : function (dataProps) {
        function foundMatch (srcTrace, tstTrace) {
            if (srcTrace.isCustom) {
                return (dataProps.viewName == tstTrace.viewName &&
                        dataProps.displayColumnName == tstTrace.displayColumnName);
            }
            else {
    ...
    rovOptionsClicked : function(e) {
        ...
        if (dialog) {
            ...
            this.$.storageOptionsSubMenu.close();
            dialog.open();
            dialog.style.zIndex = String(this.getMaxZindex() + 1);
...
//            this.$.storageOptionsDialog.disabled = !this.hasViewsStorage();
    ...
    saveViewsState : function (e) {
        ...
        this.rovInit.inFullScreenMode = this.inFullScreenMode;
        rovUtils.setLocalStorageObject('rovInit-storage', this.rovInit);
        var stateArr = [];
        for (var i = 0; i < this.rovModuleViews.length; i++) {
            var elem = this.getViewById(this.rovModuleViews[i].id);
    ...
    toolbarResponseToResize : function (narrow) {
        ...
        if (!narrow) {
            ...
            this.$.dashboardDiv.hidden = panelWidth < (620 + addOnsButtonWidth);
            ...
            this.$.rovOptionsButton.hidden = false;
            this.$.dashboardDiv.hidden = panelWidth < (435 + addOnsButtonWidth);
            this.$.dashboardOverflowDialogButton.hidden = !this.$.dashboardDiv.hidden;
    ...
    viewFromGraphOrCustom : function(moduleName, viewName, viewArgs, rowKeys) {
        ...
        if (!found) {
            /* open a view */
            var offset = this.getNewClickedViewPosition();
            var columnVisible = this.columnIsVisibleFromStorage(moduleName, viewName, rowKeys.selectedColumnName);
